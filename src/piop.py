...
class AirTable:
    ...
    preprocessed_columns: List[List[Fp]]
...
def column_up(col: List[Fp]) -> List[Fp]:
    up = copy.deepcopy(col)
    up[-1] = up[-2]
    return up


def column_down(col: List[Fp]) -> List[Fp]:
...
def sumcheck_verify(
    ...
    degree: int,
    n_vars: int
) -> Tuple[Fp, Evaluation]:  # (sum, delayed evaluation)
    challenges = []
    sum = None
    target = None
    for i in range(n_vars):
        poly = UnivariatePolynomial(fs.receive_scalars(degree + 1))
        if i == 0:
            sum = poly.evaluate(Fp(0)) + poly.evaluate(Fp(1))
        else:
            assert target == poly.evaluate(Fp(0)) + poly.evaluate(Fp(1))
...
def piop_verify(table: AirTable, proof_transcript: List[Fp]):
    ...
    assert zero_sum == 0
    witness_shifted_evals = fs.receive_scalars(table.n_witness_columns() * 2)
    ...
    up = ArithmeticCircuit.matrix_up_lde.evaluate(matrix_lde_point)
    down = ArithmeticCircuit.matrix_down_lde.evaluate(matrix_lde_point)

    final_inner_claims = fs.receive_scalars(table.n_witness_columns())
    batched_inner_value = Fp(0)
    ...
    batched_inner_value *= MultilinearEvals([Fp(1) - zerocheck_eval[0], zerocheck_eval[0]]).evaluate(inner_sumcheck_challenge.point[:1])
    ...
    packed_value = MultilinearEvals(final_inner_claims + [Fp(0)
